...
boolean smog_updisp_cf1 = true;
boolean smog_max_updisp_cf1 = true;
boolean smog_min_updisp_cf1 = true;
...
void Displaylabel_1(){
    ...
    myGLCD.print("SmogMirror:ug/m3", LEFT, 0, display_rot);    
    myGLCD.setBackColor(0,0,205);
    myGLCD.print("PM1.0", LEFT, 16, display_rot);   
    myGLCD.setBackColor(139,69,19);
    myGLCD.print("PM2.5", CENTER, 16, display_rot);   
    myGLCD.setBackColor(130,57,53);
    myGLCD.print(" PM10", RIGHT, 16, display_rot);  
    myGLCD.setBackColor(0,102,102);
    myGLCD.print("   1 hour AVG   ", CENTER, 44, display_rot);    
    myGLCD.setBackColor(20,68,106);
    myGLCD.print("     Maximum    ", CENTER, 72, display_rot);   
    myGLCD.setBackColor(3,38,58);
    myGLCD.print("     Minimum    ", CENTER, 100, display_rot);   
}*/
void Displaylabel_2(){
    myGLCD.setFont(SmallFont);          //设置字体为SmallFont格式(8*12字符)
    myGLCD.setBackColor(0,0,0);
    myGLCD.print("SmogMirror:count", LEFT, 0, display_rot);    
    myGLCD.print(">0.3um", RIGHT, 16, display_rot);         
    myGLCD.print(">0.5um", RIGHT, 36, display_rot);         
    myGLCD.print(">1.0um", RIGHT, 56, display_rot);         
    myGLCD.print(">2.5um", RIGHT, 76, display_rot);         
    myGLCD.print(">  5um", RIGHT, 96, display_rot);         
    myGLCD.print("> 10um", RIGHT, 116, display_rot);         
}
...
void DisplayUpdate(){
  if((lowByte(received_pkg) & 0x3F) == 0){
    ...
    smog_min_updisp=true;
    Displaylabel();
  } else if ((lowByte(received_pkg) & 0x3F) == 40){
    myGLCD.clrScr(); //清屏
    smog_max_updisp=true;
    smog_min_updisp=true;
    Displaylabel_2();  
  ...
  if((lowByte(received_pkg) & 0x3F) < 40){
    DisplayPM();
  } else {
    DisplayPM_2();
  }
