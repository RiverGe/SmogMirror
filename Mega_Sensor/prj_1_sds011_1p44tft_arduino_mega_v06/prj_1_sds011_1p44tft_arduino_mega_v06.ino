...
{ 
  ...
  myGLCD.print("Smog Mirror V0.6 ", LEFT, 65, display_rot);    
  ...
  myGLCD.print("Temp(oC)", LEFT, 44, display_rot);   
  ...
  while(1) {
    ...
    */  
    // Update the display
    if(smog_updisp || temp_humid_updisp){
      DisplayUpdate();
      smog_updisp=false;
...
void DisplayUpdate(){
  ...
  if (lowByte(received_pkg) > UPDATE_WINDOW){
    index=(unsigned int) (pm25/50);
    index = index >9? 9 : index;  
  } else {
//  index = random(10)%9;
    index = (received_pkg)%10;
  }
  myGLCD.setColor(smog_front_r[1], smog_front_g[1], smog_front_b[1]);//设置字体颜色
  myGLCD.setBackColor(smog_back_r[index],smog_back_g[index], smog_back_b[index]);//设置背景颜色
  ...
  if (lowByte(received_pkg) > UPDATE_WINDOW){
    index=(unsigned int) (temperature/5);
    index = index >9? 9 : index;  
  } else {
    index = (received_pkg+1)%10;
  } 
  myGLCD.setColor(smog_front_r[0], smog_front_g[0], smog_front_b[0]);//设置字体颜色
  myGLCD.setBackColor(smog_back_r[index],smog_back_g[index], smog_back_b[index]);//设置背景颜色 
  myGLCD.printNumI((int)temperature, LEFT, 28,  4, ' ');
  
  if (lowByte(received_pkg) > UPDATE_WINDOW){
    index=(unsigned int) (humidity/10);
    index = index >9? 9 : index;  
  } else {
    index = (received_pkg+2)%10;
  } 
  myGLCD.setColor(smog_front_r[0], smog_front_g[0], smog_front_b[0]);//设置字体颜色
  myGLCD.setBackColor(smog_back_r[index],smog_back_g[index], smog_back_b[index]);//设置背景颜色 
  myGLCD.printNumI((int)humidity, RIGHT, 28, 4, ' ');
